...
MetricInfoArtifact = namedtuple("MetricInfoArtifact",
                                ["model_name","model_object","train_prec","test_prec",
                                 "train_accuracy","test_accuracy","model_accuracy","index_number","overfit_score"])


def evaluate_classification_model(model_list:list,X_train:np.ndarray, y_train:np.ndarray,X_test:np.ndarray,
                                  y_test:np.ndarray,base_accuracy:float,overfit_score:float) -> MetricInfoArtifact:
    """
    Description:
    ...
    X_test : Testing dataset input features
    y_test : Testing dataset input features
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"{train_rec}\t\t {test_rec}\t\t")
            ...
            logging.info(f"{train_prec}\t\t {test_prec}\t\t")
            ...
            logging.info(f"{'>>'*30} Score {'<<'*30}")
            logging.info(f"Train f1\t\t Test f1\t\t")
            logging.info(f"{train_f1}\t\t {test_f1}\t\t")

            # Calculating model accuracy and overfitting/underfitting extent
            train_acc = accuracy_score(y_train,y_train_pred)
            test_acc = accuracy_score(y_test,y_test_pred)

            model_accuracy = np.average([train_acc,test_acc])
            ...
            logging.info(f"Train root mean squared error: [{train_prec}]")
            logging.info(f"Test root mean squared error: [{test_prec}]")

            # if model accuracy is greater than base accuracy and train and test score is within certain threshold, we will accept the model as accepted model

            """
            MetricInfoArtifact = namedtuple("MetricInfoArtifact",
            ["model_name","model_object","train_prec","test_prec","train_accuracy","test_accuracy",
            "model_accuracy","index_number","overfit_score"])
            """
...
class ModelFactory:
    ...
    def class_for_name(module_name:str, class_name:str):
        try:
            # load the module,will raise ImportEroor if module cannot be loaded
    ...
    def execute_grid_search_operation(self,initialized_model: InitializedModelDetail, input_feature,
        ...
        """"
        execute_grid_search_operation() : function will perform parameter search operation and it will
        ...
        output_features : Target/Dependent features
        ============================================================================================
    ...
    def initiate_best_parameter_search_for_initialized_models(self,
        ...
        try:
            ...
            for initialized_model_list in initialized_model_list:
                grid_searched_best_model = self.initiate_best_parameter_search_for_initialized_models(
